...
#mkdir scan hdr mhdr
#for i in 0* Bg; do zip scan/$i.zip `ls $i/*|grep -v \\\\.mhdr\\\\.|grep -v \\\\.hdr\\\\.`; done
#for i in 0* Bg; do zip hdr/$i.zip `ls $i/*|grep -v \\\\.mhdr\\\\.|grep -v \\\\.scan\\\\.`; done
#for i in 0* Bg; do zip mhdr/$i.zip `ls $i/*|grep -v \\\\.hdr\\\\.|grep -v \\\\.scan\\\\.`; done
